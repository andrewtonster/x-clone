...
function PostInteractions({
  ...
  /* setting state for if current user interacted and how many likes */
  ...
  const likeAction = async () => {
    ...
    setState((prev) => {
      // set the state to everything it previously was but update likes
      return {
        ...prev,
  ...
  // pass the current state of the post interactions (likes, repost, save)
  const [optimisticCount, addOptomisticCount] = useOptimistic(
    state,
    (prev, type: "like" | "rePost" | "save") => {
      // define the function parameters for the addOptimistic count "like", "rePost", "save"
      // determines which aciton
      if (type === "like") {
        return {
          ...prev,
          likes: prev.isLiked ? prev.likes - 1 : prev.likes + 1,
  ...
  return (
    <div className="flex items-center justify-between my-2 text-textGray gap-4 lg:gap-2 xl:gap-16">
